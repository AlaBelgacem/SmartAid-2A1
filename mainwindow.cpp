...
//************************************************  ARDUINO  *********************************************************************
    ...
    case(-1):qDebug() << "arduino is not available";
     ...
     QObject::connect(A.getserial(),SIGNAL(readyRead()),this,SLOT(on_ard_butt_clicked())); // permet de lancer
...
{
    Evenements E;
    QString value=ui->id_cert->text(); //Declaration de la variable qui va contenir l'id saisi
    ...
    QString html =
        ...
        "<span style='font-size:50px;font-family:Monotype Corsiva,serif; font-weight:bold'>"+qry.value(1).toString()+"</span>"
        "<br><br><br><br>"
        ...
        " </div>"
        "<div align =right>"
        "<span  style='font-size:15px'>"+qry.value(2).toString()+" , Le "+qry.value(5).toDate().toString()+"</span>"
        " </div>"
        " </div>"
        "</div>";
...
void MainWindow::on_ard_butt_clicked()
{

QSqlQuery qry;
qry.prepare("select * from coffre");
if(qry.exec())
{
  while(qry.next())
  {
...
ui->ard2->setText(data);
ui->ard3->setText(qry.value(0).toString());
if(data==qry.value(0).toString())
{ui->ard->setText("Correct");
     A.write_to_arduino("1");

}
else
{ui->ard->setText("Inorrect");
